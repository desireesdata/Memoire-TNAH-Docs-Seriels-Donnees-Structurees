...
Sur le plan technique, la vérité terrain est lié au schéma de données (voir Fig. \ref{fig:modelpy}). Le schéma de données fait office de \enquote{carte} qui vient guider la production textuelle du LLM. Il indique \enquote{où} mettre les bons mots dans les bonnes boîtes, les bons intitulés avec les bonnes clés. Pour éviter de démultiplier les paramètres à tester, le prompt (voir annexe~\ref{ann:prompt}) et le schéma (voir annexe~\ref{ann:schema}) ont été stabilisés à l'issu d'un processus itératif pour évaluer la granularité adéquate. Dans le cadre de l'évaluation, seul le nom du sénateur et les pages de références ont été retenues. Pour guider au mieux la génération du JSON par le LLM, on ajoute à notre schéma des descriptions pour qu'il puisse être plus attentif pour qu'il puisse plus facilement mettre les données dans les bonnes boîtes (Voir Fig.\ref{fig:model}).
...
Le schéma joue donc ici le rôle de grammaire opérationnelle : il décrit précisément la structure attendue des sorties (nom, rôle et références paginaires des intervenants), et contraint le modèle à produire un JSON validable. Les premières clés (« Senateur », « ListeSenateur ») ont rapidement montré leurs limites : elles risquaient l'invisibilité d’autres acteurs -- comme les ministres -- et biaisaient l’extraction. La version finale de la vérité terrain (voir annexe~\ref{ann:pred2}) adopte des catégories plus neutres (« Intervenant »), révélant que la modélisation n’est jamais un simple exercice technique mais un choix interprétatif.

Quant aux données produites automatiquement par cette chaîne de traitement stabilisées et appliquées sur l'ensemble des pages des \emph{Tables} du Sénat de 1931 (voir annexe~\ref{ann:vt2}) , elles semblent être cohérentes. Mais, à défaut d'évaluation, on ne pas tirer de conclusion décisives.
...
L’objectif du protocole d’évaluation est de comparer les sorties produites par le modèle de langage, notées \emph{P} pour \emph{prédiction}, avec le \emph{ground truth}, c’est-à-dire la vérité terrain notée \emph{G}. Chaque instance de donnée correspond à une liste d’entrées, constituée d’un nom d’orateur et d’une liste de pages référencées. Le principal défi de cette comparaison tient au fait que le modèle peut générer l’ensemble correct d’entrées, mais dans un ordre différent, ou avec des variations structurelles mineures. Afin de résoudre cette difficulté, la stratégie retenue repose sur un alignement flexible, inspiré du transport optimal, qui établit une correspondance injective \emph{one-to-one} entre les prédictions et les données de référence. Cette approche permet de neutraliser la contrainte d’ordre, tout en tolérant des divergences mineures, et garantit une évaluation robuste et reproductible.
