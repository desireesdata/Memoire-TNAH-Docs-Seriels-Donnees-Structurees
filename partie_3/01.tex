\chapter[L'outil Corpusense]{L'outil Corpusense : une chaîne de traitement pour les sources historiques}
...
L’objectif de Corpusense veut fournir une infrastructure technique robuste pour solidariser les différents rouages (à savoir la constitution des corpus, OCR, extraction) (voir Fig. \ref{fig:pipieline_mezz}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{pipieline_mezz.png}
\caption{Pipeline qui commence avec des documents sous format image et conduit jusqu'à des données structurées sous format JSON}
...
L’expérimentation menée s’est organisée autour de deux volets complémentaires : d’une part, la génération automatique de données à partir des \emph{Tables} parlementaires proprement dite; et, d’autre part, leur évaluation à l’aune d’une vérité terrain soigneusement construite.
...
L'extraction d'une réponse dans l'environnement d'un \emph{chat} résiste à l'extraction systématique, notamment parce qu'elle est entourée de politesses qui se surajoutent à l'information  qui nous intéresse -- ici la liste des sénateurs (voir Fig. \ref{fig:mistral} ). Mais également parce que cette réponse est émise dans un environnement -- en ligne -- qu'il est difficile à intégrer dans une pipeline. Les appels à l'API Mistral permettent donc à la fois d'interroger un LLM comme on l'a fait, mais depuis un script qui peut récupérer les données, les sauvegarder en local, d'appliquer divers traitements.

Ce qui nous permet désormais de justifier l’usage de Mistral : l’appel à l’API était gratuit, sans restrictions, et supportait la génération par sortie structurée -- c’est-à-dire la capacité de contraindre le modèle à suivre un schéma formel représentant nos données. Autrement dit, on peut utiliser, sans aucune installation ou dépense, un LLM pouvant être \enquote{contraint} à formuler du JSON -- et rien que du JSON -- par un schéma formels afin de produire des objets comparables. De fait, ce schéma (voir Fig. \ref{fig:modelepy}) était produit avec la bibliothèque logicielle Pydantic qui permet de convertir des classes Python qui représentent le schéma de données, en modèle JSON.
...
Un second volet des expérimentations a porté sur la construction de la vérité terrain, élément indispensable à toute évaluation. En théorie, elle sert de référence absolue pour mesurer les performances du modèle. En pratique, elle résulte d’une série de décisions éditoriales et méthodologiques qui influencent directement les résultats. L’exemple de Louis Barthou, sénateur et ministre de la Guerre en 1931, illustre ce point : fallait-il regrouper toutes ses interventions sous une seule entrée ou distinguer ses apparitions selon ses fonctions (voir Fig. \ref{fig:barthou} ) ? Le choix de la granularité maximale – séparer chaque occurrence – prévient les confusions d’homonymes, mais complique l’évaluation en raison de la tendance du modèle à fusionner les mentions.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{barthou.jpg}
\caption{Fragment du Journal Officiel. Une même personne peut avoir plusieurs entrées.}
\label{fig:barthou}
\end{figure}

Sur le plan technique, la vérité terrain est lié au schéma de données. Le schéma de données fait office de \enquote{carte} qui vient guider la production textuelle du LLM. Il indique \enquote{où} mettre les bons mots dans les bonnes boîtes, les bons intitulés avec les bonnes clés. Pour éviter de démultiplier les paramètres à tester, le prompt (voir annexe ~\ref{lst:prompt}) et le schéma (voir annexe~\ref{lst:schema}) ont été stabilisés à l'issu d'un processus itératif pour évaluer la granularité adéquate. Dans le cadre de l'évaluation, seul le nom du sénateur et les pages de références ont été retenues. Pour guider au mieux la génération du JSON par le LLM, on ajoute à notre schéma des descriptions pour qu'il puisse être plus attentif pour qu'il puisse plus facilement mettre les données dans les bonnes boîtes (Voir Fig.\ref{fig:model}).
...
\subsection{Des pages aux dates : utilisation de l'API Gallica et des métadonnées des \emph{manifests}}

La constitution d’un tableau « page → date » s’appuie sur les services IIIF proposés par Gallica, qui fournissent pour chaque document numérisé un \emph{manifest} JSON décrivant l’ensemble de ses pages. Ce \emph{manifest} joue le rôle de table des matières numérique : il contient, pour chaque page, son ordre dans le volume, son libellé (numéro de page ou folio) et des identifiants pérennes vers l’image et le contenu OCRisé. La première étape consiste à interroger automatiquement cette ressource -- via des requêtes HTTP en lignes de commande, et notamment avec \enquote{curl} -- pour extraire la séquence complète des pages.

Ensuite, les pages sont indexées : chaque entrée du \emph{manifest} est associée à son URL lisible et à son numéro de séquence. Cette structure séquentielle est cruciale, car elle fournit un pivot neutre pour relier des références issues d’autres sources (tables imprimées, index, etc.).
...
En combinant ces deux sources (\emph{manifest} + extraction de dates), on construit une table de correspondance exhaustive reliant chaque page d’un volume à sa date exacte de parution. Cette table, exportée en CSV, sert ensuite de base à des analyses historiennes : une référence de page extraite d’un index thématique peut être immédiatement replacée dans son contexte temporel. L’approche, entièrement automatisable, repose sur des standards ouverts (IIIF, JSON, HTTP) et sur des outils simples de parsing et de jointure de données, ce qui garantit sa robustesse et sa réutilisabilité (voir annexe ~\ref{lst:vt2}).
...
L’objectif du protocole d’évaluation -- qui a d'ailleurs fait l'objet d'un article scientifique soumis et accepté pour le \emph{Computational Humanities Research} 2025
\footnote{L’article s’intitule \enquote{\mkbibemph{Producing Structured Data from Historical Sources: a Preliminary Application to French Senate Tables}}, co-écrit avec Marie Puren et Joseph Chazalon. Voir : \url{https://2025.computational-humanities-research.org/}. (Publication à venir.)}
 -- est de comparer les sorties produites par le modèle de langage, notées \emph{P} pour \emph{prédiction}, avec le \emph{ground truth}, c’est-à-dire la vérité terrain notée \emph{G}. Chaque instance de donnée correspond à une liste d’entrées, constituée d’un nom d’orateur et d’une liste de pages référencées. Le principal défi de cette comparaison tient au fait que le modèle peut générer l’ensemble correct d’entrées, mais dans un ordre différent, ou avec des variations structurelles mineures. Afin de résoudre cette difficulté, la stratégie retenue repose sur un alignement flexible, inspiré du transport optimal, qui établit une correspondance injective \emph{one-to-one} entre les prédictions et les données de référence. Cette approche permet de neutraliser la contrainte d’ordre, tout en tolérant des divergences mineures, et garantit une évaluation robuste et reproductible.

La comparaison est strictement indépendante de la phase d’OCR, qui constitue une étape préalable. Chaque page est d’abord traitée par le moteur PERO, qui assure la détection et la transcription des blocs textuels. Compte tenu des difficultés récurrentes de segmentation dans la mise en page, trois variantes de transcription sont produites afin de refléter la variabilité propre à ce type de pipeline : une version brute, sans segmentation, qui restitue le flux textuel tel quel ; une version corrigée, reposant sur une segmentation manuelle ; et enfin une émulation d’OCR parfait, réalisée à la main, qui sert de vérité terrain. Dans le cadre de cette expérimentation, chaque page a été traitée indépendamment pour des raisons pratiques, mais la méthode est extensible à des contextes multi-pages.

Un autre point crucial concerne la contrainte du format de sortie des modèles de langage. Si ceux-ci peuvent être incités à produire des données structurées à l’aide de prompts adaptés, il est indispensable de s’assurer que les réponses respectent strictement le schéma attendu. Parmi les méthodes disponibles, l’une des plus efficaces consiste à filtrer les tokens valides au moment de l’inférence, en s’appuyant sur un validateur externe tel qu’un automate fini. Cette capacité, aujourd’hui intégrée à certaines API commerciales, permet de garantir la conformité des résultats et d’éviter les dérives fréquentes liées à la génération libre.

Le processus d’extraction mis en œuvre dans cette étude repose ainsi sur une combinaison articulée de quatre éléments : un texte prétraité par OCR, un schéma prédéfini exprimé en JSON, un prompt en langage naturel destiné à orienter la tâche, et enfin une API supportant la contrainte de sortie. Pour les expérimentations, l’API Mistral a été retenue, et plus spécifiquement le modèle \emph{Ministral 8B Instruct v2410}. Ce choix s’explique par ses performances satisfaisantes en \emph{zero-shot}, son coût modéré, mais aussi par la disponibilité publique de ses poids, qui garantit une réutilisation et une reproductibilité dans un cadre de recherche.

\subsection{Vérité terrain, prompt et schéma : un échantillon des Tables}

A l'issue de la démarche d'expérimentation en vue de l'évaluation a été produit le design de vérité terrain, accompagné de son schéma et du prompt, qui sera comparées à l'information extraite par le LLM. La tâche d’extraction repose sur l’identification des entrées individuelles dans les \emph{Tables nominatives} — c’est-à-dire les noms des sénateurs et leurs références de pages. (Voir annexe~\ref{ann:B}, n°~\ref{lst:vt2})
...
\section[Une métrique d'évaluation]{Une métrique pour l'évaluation des données générées par la sortie stucturée via LLM}

\subsection{Mise en correspondance des prédictions et de la vérité terrain avec le transport optimal}

L’évaluation des données produites par le modèle repose sur une mise en correspondance entre les entrées prédites (\emph{P}) et celles de la vérité terrain (\emph{G}). Chaque entrée est composée de deux éléments : un nom d’orateur et une liste de pages référencées. Pour mesurer la proximité entre deux entrées, nous définissons une distance normalisée, notée $d_e(g_i, p_j)$, qui combine deux composantes complémentaires.

La première composante concerne le nom de l’orateur. Afin de prendre en compte les variations mineures d’orthographe, de casse ou d’espacement, nous utilisons la distance de Ratcliff/Obershelp. Celle-ci repose sur l’identification de la plus longue sous-chaîne commune entre deux chaînes de caractères. La mesure est normalisée entre 0 et 1 : elle vaut 0 lorsqu’il y a correspondance exacte, et tend vers 1 lorsque les chaînes n’ont aucun segment significatif en commun.

La seconde composante concerne les pages référencées, qui sont représentées sous forme d’ensembles. Pour comparer ces ensembles, nous utilisons la distance dérivée de l’Intersection-over-Union (IoU), une mesure couramment utilisée en vision par ordinateur pour évaluer le chevauchement entre deux zones détectées. Dans notre cas, il s’agit de calculer la proportion de pages communes entre une prédiction et la vérité terrain, rapportée à l’ensemble des pages mentionnées par l’un et l’autre. La formule retenue est la suivante :

$$
d_p(g_i, p_j) = 1 - \frac{|ref\_pages(g_i) \cap ref\_pages(p_j)|}{|ref\_pages(g_i) \cup ref\_pages(p_j)|}
$$

Cette distance prend la valeur 0 lorsque les ensembles de pages sont identiques, et tend vers 1 lorsque les deux ensembles n’ont aucune page en commun. Une similarité parfaite (IoU = 1, donc $d_p = 0$) signifie que le modèle a identifié exactement les mêmes pages que la vérité terrain ; une absence totale d’intersection ($IoU = 0$, donc $d_p = 1$) signale une divergence complète.

La distance entre deux entrées est finalement calculée en combinant ces deux mesures élémentaires à l’aide d’un produit, ce qui permet de conserver un domaine de définition compris entre 0 et 1. Cette distance globale est définie comme suit :

$$
d_e(g_i, p_j) = d_n(g_i, p_j) \times d_p(g_i, p_j)
$$

Le choix du produit comme opérateur de combinaison présente un double avantage : il préserve l’interprétation probabiliste des deux mesures et pénalise fortement les cas où l’une des deux composantes est maximale. Ainsi, une prédiction qui reproduit correctement le nom mais échoue totalement sur les pages, ou inversement, se voit attribuer une distance élevée. Cette sévérité est cohérente avec nos besoins d’extraction, puisque l’objectif n’est pas seulement d’approcher la vérité terrain, mais bien de reproduire simultanément l’ensemble des attributs d’une entrée.

Un appariement \emph{one-to-one} entre prédictions et vérité terrain est ensuite établi par transport optimal, en minimisant la somme des distances globales $d_e$. Cette méthode permet d’exclure efficacement les grandes divergences et de mettre en évidence les correspondances pertinentes, même en cas de légères variations lexicales ou de différences d’ordre.

\subsection{Limites des métriques classiques : précision, rappel et F1}

Les métriques comme la précision, le rappel et la F-mesure, moyenne harmonique de la précision et du rappel, porte les mêmes défauts que les termes qui la composent, sont couramment utilisées pour évaluer les tâches d’extraction. \emph{La précision} mesure la proportion de prédictions correctes parmi toutes les prédictions effectuées ($\text{Précision} = \frac{\text{vrais positifs}}{\text{vrais positifs + faux positifs}}$), tandis que \emph{le rappel} exprime la proportion d’éléments correctement prédits parmi ceux attendus ($\text{Rappel} = \frac{\text{vrais positifs}}{\text{vrais positifs + faux négatifs}}$). La \emph{F1} est la moyenne harmonique de ces deux scores ($\text{F1} = 2 \cdot \frac{\text{Précision} \cdot \text{Rappel}}{\text{Précision}+\text{Rappel}}$), censée équilibrer exhaustivité et exactitude.
...
\subsection{Une métrique adaptée : l'Integrated Matching Quality (IMQ)}

Les métriques classiques ne parviennent pas à capturer la qualité nuancée des appariements structurés, en particulier lorsque les prédictions sont partiellement correctes mais pas parfaitement alignées avec les références. Pour répondre à cette limitation, nous proposons une métrique continue, l'Integrated Matching Quality (IMQ), qui évalue la qualité globale des appariements en intégrant une mesure de similarité sur l'ensemble des seuils possibles.

Pour chaque paire d'entrées appariées, c'est-à-dire une entrée prédite $p_i$ et une entrée de référence $g_i$, nous calculons un score de qualité $q_i$ basé sur la distance $d_e(g_i, p_i)$ que nous venons de définir. Le score de qualité est défini comme suit :

$$q_i = 1 - d_e(g_i, p_i)$$

Ce score $q_i$ reflète la proximité entre la prédiction et la référence, avec une valeur de 1 pour un appariement parfait et une valeur proche de 0 pour un appariement très éloigné.

Pour évaluer la qualité des appariements de manière globale, nous introduisons la fonction $F(t)$, qui représente la proportion des appariements dont le score de qualité $q_i$ est supérieur ou égal à un seuil $t$. Cette fonction est définie par :
$$F(t) = \frac{1}{n} \sum_{k=1}^{n} \mathbb{1}_{q_k \geq t}$$

où $n$ est le nombre total d'appariements et $\mathbb{1}_{q_k \geq t}$ est une fonction indicatrice qui vaut 1 si $q_k \geq t$, et 0 sinon. La fonction $F(t)$ permet de visualiser la distribution des scores de qualité $q_i$ sur l'intervalle $[0, 1]$. Par exemple, si tous les appariements sont parfaits, $F(t) = 1$ pour tout $t \in [0, 1]$. À l'inverse, si certains appariements sont de mauvaise qualité, $F(t)$ décroît à mesure que $t$ augmente.

L'IMQ est défini comme l'aire sous la courbe de $F(t)$ sur l'intervalle $[0, 1]$ :

$$\text{IMQ} = \int_0^1 F(t) \, dt$$

Cette intégrale résume la qualité globale des appariements en prenant en compte à la fois le nombre d'appariements de haute qualité et leur proximité avec les références. Plus il y a d'appariements avec un $q_i$ élevé, plus $F(t)$ reste proche de 1 pour des valeurs de $t$ élevées, ce qui augmente l'IMQ. Même si certains appariements ne sont pas parfaits, leur contribution à l'IMQ dépend de leur score $q_i$, ce qui permet de valoriser les appariements partiellement corrects.

L'IMQ offre une interprétation intuitive : une valeur de 1 indique que tous les appariements sont parfaits, quand une valeur proche de 0 signifie que la majorité des appariements sont de mauvaise qualité. Les valeurs intermédiaires reflètent un compromis entre le nombre d'appariements corrects et leur proximité avec les références.

L'un des principaux avantages de l'IMQ est son indépendance aux seuils arbitraires. Contrairement aux métriques binaires, l'IMQ ne dépend ainsi pas d'un seuil fixe pour juger de la qualité d'un appariement. Elle intègre la qualité sur l'ensemble des seuils possibles, ce qui la rend particulièrement adaptée aux sorties des LLMs, où des divergences mineures sont fréquentes même sous de fortes contraintes structurelles.

Un autre avantage est sa robustesse aux divergences mineures. Les petites variations, comme une étiquette prédite proche de la référence, sont moins pénalisées que dans les métriques classiques. Cela permet une évaluation plus fine et plus représentative de la performance réelle des modèles, tout en intégrant la problématique de l'appariement.

\subsection{Résultats et analyse}

Nous avons appliqué notre méthode d’appariement sur cinq pages distinctes (109 entrées), chacune traitée indépendamment et présentant des qualités OCR variables. Voici les résultats pour chaque page OCRisée sans segmentation ni correction, ce qui correspond à une situation réaliste où les données sont souvent plus bruitées et subissent moins d'étapes intermédiaires de traitement :
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{@{}lcccccc@{}}
			\toprule
			Source & Précision (biaisée) & Rappel (biaisée) & IMQ & Entrées de référence & Entrées prédites & Correspondances \\
			\midrule
	...
	\caption{Résultats des évaluations des pages avec un OCR \enquote{brut}, sans segmentation.}
	\label{tab:results}
\end{table}

Toutes les pages affichent une précision et un rappel « biaisés » parfaits, mais ces métriques sont limitées car elles découlent directement de l’appariement injectif et ne reflètent pas la qualité réelle des alignements.

L’IMQ, en revanche, fournit une évaluation plus fine, en capturant la distribution des qualités de correspondances. Malgré le bruit présent dans les données, les scores IMQ restent élevés (entre 0.8193 et 0.9591), montrant une homogénéité forte entre les correspondances et une performance proche d'une situation idéale.

Les pages 5 et 10 présentent des scores IMQ plus bas, ce qui peut être attribué à des incohérences typographiques. Sur ces pages, de nombreux prénoms ne sont pas mis entre parenthèses après le nom, contrairement à ce qui est attendu dans le \emph{ground truth} (21 \% des entrées sur la page 5, 39 \% sur la page 10). Cela augmente artificiellement la distance textuelle et dégrade la qualité perçue des correspondances.

La page 3, bien qu’affichant une précision et un rappel parfaits, montre un IMQ plus faible (0.8928), probablement en raison de problèmes OCR causés par un pli dans la reliure, générant du bruit visuel.

La page 2 affiche un IMQ élevé (0.9059) malgré un rappel imparfait. Cela s’explique par un biais d’échantillonnage : le prompt avait été calibré sur cette page, ce qui améliore artificiellement la performance. Les résultats solides sur la page 4 (IMQ = 0.9591) confirment la robustesse du dispositif.

Un cas particulier est observé sur la page 2, où une personne est mentionnée deux fois (comme sénateur et comme ministre). Le \emph{ground truth} distingue ces deux entrées, tandis que le LLM les fusionne. Cela réduit artificiellement le rappel mais correspond à une rationalisation fonctionnelle du modèle.
...
\begin{table}[h]
	...
	\caption{Résultats des évaluations des pages avec une qualité OCR optimale.}
	\label{tab:optimal_ocr_results}
\end{table}


Les résultats obtenus avec une segmentation manuelle, censée être de qualité intermédiaire entre l'OCR brut et l'OCR parfait, sont les suivants :
...
\begin{table}[h]
	...
	\caption{Résultats des évaluations des pages avec une qualité OCR intermédiaire.}
	\label{tab:intermediate_ocr_results}
\end{table}


Cette analyse qualitative montre que la qualité des données d'entrée a un impact significatif sur les performances du modèle. Dans un contexte réaliste, où les données sont souvent plus bruitées et subissent moins d'étapes intermédiaires de traitement, les résultats restent néanmoins bons et proches d'une situation \enquote{parfaite}. Cela confirme que, même avec des données moins traitées, le modèle parvient à maintenir une performance élevée.

Le prompt apparaît comme un paramètre critique qui n'a cependant pas été pris en compte dans cette évaluation initiale. En effet, le choix des instructions et leur formulation influencent directement la capacité du modèle à extraire et structurer les informations de manière cohérente. La poursuite du travail devra intégrer une évaluation systématique des variations de prompt afin de mesurer leur impact sur les performances du modèle. Cela permettra d'identifier les formulations les plus efficaces et de mieux comprendre comment le modèle interprète les consignes données.

Quant au schéma, sa conception avec une granularité raisonnable, couplée à un prompt bien défini, permet d’obtenir des résultats fiables sans nécessiter une connaissance \enquote{atomique} des spécificités documentaires. Cette approche offre une flexibilité précieuse, car elle réduit la dépendance à une expertise détaillée sur les particularités typographiques ou institutionnelles des documents. Ainsi, même dans des contextes où les données sont bruitées ou imparfaitement structurées, le modèle parvient à produire des résultats exploitables, tout en restant robuste face aux variations mineures de format.

Cette combinaison de schéma et de prompt générique ouvre la voie à une automatisation plus large de l'extraction d'informations, tout en limitant les besoins en prétraitement manuel des données. Cela représente un avantage significatif pour des applications à grande échelle, où la diversité des sources et des formats peut rendre une approche trop spécifique difficilement généralisable.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% OCR brut
			\addplot+[mark=*, thick]
			coordinates {(1,0.9059) (2,0.8928) (3,0.9591) (4,0.8636) (5,0.8193)};
			\addlegendentry{OCR brut}
			
			% OCR parfait (corrigé)
			\addplot+[mark=square*, thick, dashed]
			coordinates {(1,0.9513) (2,0.9430) (3,0.9821) (4,0.8778) (5,0.8966)};
			\addlegendentry{OCR parfait}
			
			% Segmentation manuelle (intermédiaire)
			\addplot+[mark=triangle*, thick, dotted]
			coordinates {(1,0.9016) (2,0.9395) (3,0.9784) (4,0.8793) (5,0.8873)};
			\addlegendentry{Segmentation manuelle}
			
		\end{axis}
	\end{tikzpicture}
	\caption{Scores IMQ par page selon la qualité d’OCR/segmentation.}
	\label{fig:imq_par_page}
\end{figure}


\section{Conclusion}

Ce travail a exploré l’utilisation des grands modèles de langage pour la génération de données structurées à partir de sources historiques, à travers une étude de cas centrée sur les \emph{Tables nominatives} du Sénat français de 1931. L’approche — combinant OCR, structuration guidée par schéma et génération contrainte via LLM — a produit des résultats globalement satisfaisants, comme en témoignent les scores IMQ élevés, souvent proches de 0.9. Ces résultats démontrent que la méthode est capable de gérer des données bruitées tout en maintenant une qualité d'extraction élevée, validant ainsi son efficacité dans un contexte réaliste.

L’introduction de la métrique IMQ s’est révélée essentielle : elle permet d’évaluer la qualité de structuration au-delà des scores classiques de précision/rappel, inadéquats dans ce contexte. Grâce à cette métrique, nous avons pu capturer des nuances dans la qualité des appariements, offrant une évaluation plus fine et plus représentative des performances réelles du modèle.

Plusieurs pistes s’ouvrent pour renforcer la robustesse et la généralisation de l’approche. Tout d'abord, il s'agirait de relier plus directement données extraites et questions de recherche pour s'assurer que les hypothèses de réponse formulées à partir des données générées restent robustes dans le temps. Cela permettrait de mieux ancrer les résultats dans des problématiques historiques concrètes. Ensuite, évaluer le prompt lui-même : cette étape reste à formaliser pour parvenir à un protocole d’évaluation véritablement complet. Une analyse systématique des variations de prompt pourrait révéler des axes d’amélioration significatifs. Enfin, repenser la structuration de données : elle ne doit pas être considérée comme un simple prétraitement neutre, mais comme un choix déterminant pour les analyses historiques possibles. La manière dont les données sont structurées influence directement les types de questions qui peuvent être posées et les réponses qui peuvent être obtenues.

De ce point de vue, le prompt et le schéma de données apparaissent comme des méta-paramètres du système de production de données historiques. Leur génération et leur ajustement doivent être conçus comme faisant partie intégrante de la chaîne de traitement. Une optimisation rigoureuse de ces éléments pourrait améliorer encore la qualité et la fiabilité des données produites. Une poursuite prometteuse à ce travail consisterait donc à systématiser et automatiser ce processus de méta-optimisation, afin de rendre ces approches reproductibles, transparentes et accessibles à des utilisateurs non spécialistes. Cela permettrait de démocratiser l’utilisation de ces techniques avancées dans des contextes variés, tout en garantissant une qualité constante des résultats. Ce travail montre que l’utilisation des LLM pour la structuration de données historiques est non seulement faisable, mais aussi efficace, ouvrant ainsi de nouvelles perspectives pour la recherche en sciences humaines.
